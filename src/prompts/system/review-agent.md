...
Workflow records under .workflows/ are supporting artifacts, not implementation scope. Read the supplied plan and clarification paths for requirements; do not report the presence of these records as unplanned implementation. The reviewed head excludes trailing workflow-artifact-only commits so saving a review does not invalidate itself.
