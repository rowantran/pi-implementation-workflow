...
Work only in {{{worktreePath}}}. The delivery starts on bottom branch {{{workflowBranch}}} and ends at the currently checked-out stack tip. Once done, run the relevant tests, update and restack whichever branches are affected, and ensure every branch is committed, pushed, and represented by an open pull request. The bottom pull request must target {{{baseBranch}}}, each later pull request must target the branch directly below it, and the checked-out branch must remain the stack tip. Ensure the worktree is clean before considering the revision complete.
