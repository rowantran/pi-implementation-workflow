...
export async function generateWorkflowReview(
	...
	const runAgentWithPlan: ReviewAgentRunner = (request) => runAgent({
		...request,
		prompt: `${request.prompt}\n\n${request.role === "testing-criteria" ? reviewPlanContext(input.scope, inputs, false) : approvedPlanContext}`,
	});
			...
			scope = await runTrackedAgent(scopeAgent, async () => {
				const value = await runAgentWithPlan({
					...
					prompt: incrementalReviewScopePrompt({
						...
						planPath: input.scope.currentPlan.path,
			...
			return runTrackedAgent(testingCriteriaAgent, async () => {
				const value = await runAgentWithPlan({
					...
					prompt: testingCriteriaReviewPrompt({
						testingCriteria: reviewTestingContent(inputs),
						metadataPath: input.metadataPath,
						clarificationsPath: input.clarificationsPath,
						planPath: input.scope.currentPlan.path,
						baseCommit: input.baseCommit,
...
function reviewPlanContext(scope: WorkflowScope, inputs: ReviewInputsSnapshot, includeTesting = true): string {
	const plan = scope.currentPlan.document;
	return [
		...
		...inputs.plannedChanges.flatMap((change, index) => [
			...
			...(change.kind === "followup" && change.effect.type === "amendment" ? [`Amends: ${JSON.stringify(change.effect.requirements)}`] : []),
			"", change.content,
		]),
		...(includeTesting ? ["", "Testing:", reviewTestingContent(inputs)] : []),
		"</approved-plan-evidence>",
	].join("\n");
}

function reviewTestingContent(inputs: ReviewInputsSnapshot): string {
	return inputs.testingGroups.map(({ sourceId, criteria }) => `Testing source ID: ${sourceId}\n${criteria}`).join("\n\n");
}
