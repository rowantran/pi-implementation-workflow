...
function runnerFor(input, override = async () => undefined) {
  ...
  const run = async (request) => {
    ...
    try {
      assert.ok(request.prompt.includes(input.metadataPath));
      assert.ok(request.prompt.includes(input.clarificationsPath));
      ...
      for (const change of inputs.plannedChanges) {
        ...
        for (const id of change.dependsOn) assert.ok(request.prompt.includes(id));
        if (change.kind === "followup" && change.effect.type === "amendment") assert.ok(request.prompt.includes(JSON.stringify(change.effect.requirements)));
      }
      for (const group of inputs.testingGroups) {
        assert.ok(request.prompt.includes(group.sourceId));
