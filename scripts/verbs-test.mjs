...
function assertImplementationContext(message, files, { currentVersion = 1 } = {}) {
	assert.ok(message.includes(files.metadata));
	assert.ok(message.includes(join(files.versions, `v${currentVersion}`)), "the agent receives the selected immutable plan, not a moving alias");
}
...
try {
	...
	// /workflow-review checks the live delivery, generates, reuses, re-reviews incrementally, and falls back.
	{
		...
		for (const { prompt } of harness.reviewRequests) {
			assert.ok(prompt.includes(join(workflow.files.versions, `v${firstReport.currentPlanVersion}`)), "reviewers receive the snapshot recorded in their report");
		...
		assertImplementationContext(harness.userMessages.at(-1), workflow.files, { currentVersion: 2 });
		...
		const secondReport = await storage.readWorkflowReview(workflow.files);
		assert.equal(secondReport.headCommit, "head222");
		assert.equal(await storage.pathExists(join(workflow.files.reviews, "0002.json")), true);
		for (const { prompt } of harness.reviewRequests) {
			assert.ok(prompt.includes(join(workflow.files.versions, `v${secondReport.currentPlanVersion}`)));
		...
		const followupContext = { currentVersion: 3 };
