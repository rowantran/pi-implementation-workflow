  ...
  changes: [
    {
      ...
      content: 'Keep the goal, introduction, testing criteria, and each change in separate Markdown files. Store titles and dependency slugs in JSON metadata.\n\n```ts\ninterface PlannedChange {\n  id: string;\n  title: string;\n  dependsOn: string[];\n  content: string;\n}\n```\n\nNo fixed prose fields are required. Headings and code examples remain ordinary content.',
    },
    {
      id: 'validate-graph', title: 'Reject invalid dependency graphs', dependsOn: ['define-document'],
      content: 'Check unknown identifiers, duplicate dependencies, self-dependencies, and cycles before publishing a version.\n\n> A diagram alone does not establish a usable execution order. A cycle leaves each change waiting for another.',
    },
    {
      id: 'render-graph', title: 'Draw the dependency graph', dependsOn: ['define-document'],
      content: 'Generate a top-down Mermaid diagram from the document. Include every node, including independent changes.\n\n```mermaid\nflowchart TD\n  Document --> Validation\n  Document --> Diagram\n```\n\nRendering is separate from authoring. Agents do not maintain diagram syntax for the authoritative dependency graph.',
    },
    {
      id: 'connect-reader', title: 'Connect the graph to the plan reader', dependsOn: ['render-graph'],
      content: 'Select a node to highlight prerequisites and downstream changes. Read the prose through the outline; keep Requires and Enables links beside each change.\n\nSee [the graph renderer](#render-graph) for rendering details. This link keeps working when the reading order changes.',
    },
    {
      id: 'guide-implementation', title: 'Guide implementation order', dependsOn: ['validate-graph'],
      content: 'Follow the approved dependencies, not numbered reading order. Independent branches can proceed without an artificial barrier, but overlapping edits still need coordination.\n\nAsk for clarification when a new prerequisite changes the approved plan.',
    },
    {
      id: 'document-format', title: 'Document the plan format', dependsOn: [],
      content: 'Explain slug identity, display numbering, arrow direction, and the guided graph section. Documentation can begin from the agreed design without waiting for the implementation branches.\n\n| Identity | Purpose |\n| --- | --- |\n| Slug | Stable links and dependencies |\n| Number | Position in this version |',
    },
    {
