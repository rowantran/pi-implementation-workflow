...
await scenario({
	...
	afterFirstStart: async (harness) => {
		...
		assert.ok(harness.sentMessages.at(-1).includes(join(files.versions, "v6")), "handoff supplies the finalized snapshot with the new followup");
		const beforeWork = await emit("before_agent_start", { systemPrompt: "Base" });
		assert.ok(beforeWork.systemPrompt.includes(join(files.reviews, "0001.json")), "implementation can locate the saved review");
		await callPlan({ action: "prepare" });
		await writeFile(followupPath, JSON.stringify({ title: followup.title, dependsOn: followup.dependsOn, implemented: true, followup: followup.followup }));
		await callPlan({ action: "finalize", expectedBaseVersion: 6, description: "Save review followups" });
		await run("workflow-implement");
		assert.ok(harness.sentMessages.at(-1).includes(join(files.versions, "v7")), "the next handoff uses the newly finalized flag snapshot");
		...
		assert.ok(resumedPrompt.systemPrompt.includes(join(files.versions, "v7")));
		assert.doesNotMatch(resumedPrompt.systemPrompt, /workflow-revise/);
		await writeFile(files.review, "unreadable latest review");
		await run("workflow-implement");
		assert.ok((await emit("before_agent_start", { systemPrompt: "Base" })).systemPrompt.includes(join(files.versions, "v7")), "an optional unreadable report must not remove implementation context");
