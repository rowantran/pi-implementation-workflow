...
Introduce one shared scope reader for implementation, briefing, review, and the dashboard. Read both the pinned original approval and the latest finalized snapshot, then select original changes plus accepted followups from that snapshot. Completed followups remain requirements; progress does not remove them from review.
