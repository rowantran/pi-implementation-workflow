...
Introduce one shared scope reader for implementation, briefing, review, and the dashboard. Read both the pinned original approval and the latest finalized snapshot, then select original changes plus accepted followups from that snapshot. Completed original changes and followups remain requirements; progress does not remove them from review. Load each active change's progress from the current snapshot, never infer original completion from its membership in the baseline.
...
procedure ReadWorkflowScope(files, metadata):
    ...
    normalize each change's missing implementation.json to Unassessed
