...
1. Call with `{"action":"prepare"}`. The tool copies the latest finalized version into `working-plan/`, or creates a skeleton for the first plan. It returns `draftPath` and `baseVersion`; `0` means no published version. Preparing again preserves existing unsaved edits, including invalid drafts. The result gives the current phase's edit permissions. During review it also returns the new-followup file layout and metadata schema, including the actual review/session origin.
...
Run this in an existing side-agent session to load the workflow's original ask, current finalized plan path, explicit user clarifications, and latest review when available. Followup amendments and flags remain in the referenced plan. The agent reads the context, confirms which workflow and plan status it loaded, then waits for your task. It does not start implementation or advance the workflow.
